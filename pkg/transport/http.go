...
func NewHTTPHandler(endpoints endpoint.Endpoints) http.Handler {
	r := mux.NewRouter()
	r.Methods("GET").Path("/endpoint1").Handler(endpoint.MakeEndpoint1Handler(endpoints.Endpoint1))
	r.Methods("GET").Path("/endpoint2").Handler(endpoint.MakeEndpoint2Handler(endpoints.Endpoint2))
